...
pcd <- read.csv("data/pulitzer-circulation-data.csv", stringsAsFactors = FALSE)
...
pcd <- pcd %>% mutate(Pulitzer.Prize.Change = Pulitzer.Prize.Winners.and.Finalists..2004.2014 - Pulitzer.Prize.Winners.and.Finalists..1990.2003)

# What was the name of the publication that has the most winners between 
# 2004-2014?
most.winners <- pcd %>% filter(Pulitzer.Prize.Winners.and.Finalists..2004.2014 == max(Pulitzer.Prize.Winners.and.Finalists..2004.2014))
print(most.winners$Newspaper)
...
five.winners <- pcd %>% filter(Pulitzer.Prize.Winners.and.Finalists..2004.2014 > 4) %>% 
  filter(Change.in.Daily.Circulation..2004.2013 == min(Change.in.Daily.Circulation..2004.2013))
print(five.winners$Newspaper)
